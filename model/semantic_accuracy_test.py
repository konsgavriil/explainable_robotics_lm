...
class TestAddFunction(unittest.TestCase):
    ...
    def test_good_cont_input_good_cont_output(self):
        df = pd.read_csv("persistance/moos_ivp_csv/inference/xarlm_causal_p2_inference.csv")
        inputs = []
        outputs = []
        for index, row in df.iterrows():
            inputs.append(f"User Query: {row['query']} Representation: {row['representation']}")
            outputs.append(f"Response: {row['response']}")
        
        result_dict = self.sa.compute(inputs, outputs)
        acc = result_dict["semantic_accuracy"] 
        prec = result_dict["semantic_precision"]
        self.assertEqual(acc, 0.995074639949617)
        self.assertEqual(prec, 0.9592083842083843)
