document.addEventListener('DOMContentLoaded', function () {
    ...
    var expNoOption = query.options[0];
    ...
    const scenarios_a = ["https://zenodo.org/records/11153372/files/m34_alpha_a.webm", "https://zenodo.org/records/11153372/files/s1_alpha_a.webm", "https://zenodo.org/records/11153372/files/s4_delta_a.webm"];
    const scenarios_b = ["https://zenodo.org/records/11153372/files/m34_alpha_b.webm", "https://zenodo.org/records/11153372/files/s1_alpha_b.webm", "https://zenodo.org/records/11153372/files/s4_delta_b.webm"];
    const substrings = ["m34_alpha_a", "m34_alpha_b", "s1_alpha_a", "s1_alpha_b", "s4_delta_a", "s4_delta_b"];
    questionButton.disabled = true;
    explanationButton.disabled = true;
    qHiddenInput = document.getElementById('questionHiddenInput');
    eHiddenInput = document.getElementById('explanationHiddenInput');
    qList = [];
    eList = [];
    var selectedOption = "";
    var selectedQuery = "";
    subs = "https://zenodo.org/api/records/11153372/files/example.vtt/content";
    loadSubtitles(subs); 
    ...
    // Add event listener for the click event on each radio button
    option1.addEventListener('click', handleRadioClick);
    option2.addEventListener('click', handleRadioClick);
    option3.addEventListener('click', handleRadioClick);

    expNoOption.addEventListener('click', handleNoQueryClick);
    expOption1.addEventListener('click', handleQueryClick);
    expOption2.addEventListener('click', handleQueryClick);
    expOption3.addEventListener('click', handleQueryClick);
    ...
    function handleNoQueryClick(event) {
        selectedQuery = "no_query";
        explanationButton.disabled = true;
    }
    ...
    video.addEventListener('ended', function () {
        if (videos.length === 0) {
            ...
            eHiddenInput.value = eList;
            console.log(qHiddenInput.value);
            console.log(eHiddenInput.value);
        }
    ...
    video.addEventListener('timeupdate', function () {
        ...
        if (video.currentTime >= timestamp) {
            ...
            qList.push(questions[qTimestampIndex]);
    ...
    function playRandomVideo() {
        ...
        if (condition == 4) { 
            ...
            if (series == 'a') { 
                var selectedVideo = "https://zenodo.org/records/11153372/files/s3_charlie_a.webm";
                ...
                var selectedVideo = "https://zenodo.org/records/11153372/files/s3_charlie_b.webm";
            ...
            switch (scenario) {
                case "s1_alpha_a":
                    ...
                    subs = "https://zenodo.org/api/records/11153372/files/s1_alpha_causal_a.vtt/content";
                    loadSubtitles(subs);    
                    ...
                    contr_query1 = ["Why not return to the starting point?", "Why not avoid obstacle A?", "Why not modify its trajectory?"];
                    contr_query2 = ["Why not stay idle?", "Why not avoid obstacle B?", "Why not move backwards?"];
                    contr_query3 = ["Why not avoid an obstacle?", "Why not avoid obstacle D?", "Why not move underwater?"];
                    contr_explanation1 = ["Alpha would need to complete the survey or be asked to return by command and control.", "Alpha has not encountered obstacle A yet.", "The vessel needs to be in medium proximity to an obstacle to alter its trajectory."];
                    contr_explanation2 = ["Alpha would have returned to the starting point or cancel its deployment to stay idle.", "Alpha has not encountered obstacle B yet.", "That's not how the vessel behaves when it surveys an area or encounters an obstacle."];
                    contr_explanation3 = ["The vessel needs to be in medium proximity to an obstacle to avoid it.", "There is no obstacle D in this mission.", "Alpha cannot move underwater since it's a surface vehicle."];
                    break;
                case "s1_alpha_b":
                    qScenarioTimestamps = [8, 22, 52];
                    subs = "https://zenodo.org/api/records/11153372/files/s1_alpha_causal_b.vtt/content";
                    loadSubtitles(subs);    
                ...
                case "s4_delta_a":
                    ...
                    subs = "https://zenodo.org/api/records/11153372/files/s4_delta_causal_a.vtt/content";
                    loadSubtitles(subs);
                ...
                case "s4_delta_b":
                    ...
                    subs = "https://zenodo.org/api/records/11153372/files/s4_delta_causal_b.vtt/content";
                    loadSubtitles(subs);
                    questions = ['What is the speed and depth of the vessel?', 'Why is Delta moving back to the loiter area?', 'Why does Delta remain stationary?'];
                    options1 = ['Low depth and moderate speed', 'Due to a time threshold', 'Reducing its depth'];
                    options2 = ['High depth and moderate speed', 'It was asked by command and control', 'GPS Fix'];
                ...
                case "m34_alpha_a":
                    ...
                    var subs = "https://zenodo.org/api/records/11153372/files/m34_alpha_causal_a.vtt/content";
                    loadSubtitles(subs);
                ...
                case "m34_alpha_b":
                    ...
                    var subs = "https://zenodo.org/api/records/11153372/files/m34_alpha_causal_b.vtt/content";
                    loadSubtitles(subs);
        ...
        eList.push(selectedVideo);
        qList.push(selectedVideo);
    ...
    function loadSubtitles(url) {
        fetch(url)
            ...
            .catch(error => {
                console.error('There was a problem with fetching the subtitles:', error);
            });
    }
});
